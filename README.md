...
コンテナ起動時に `zenn preview` が自動的にバックグラウンドで起動します。  
VS Code がポート `8000` を自動転送し、プレビューパネルが開きます。

手動で確認したい場合は、ブラウザで `http://localhost:8000` にアクセスしてください。  
プレビューサーバーのログは `/tmp/zenn-preview.log` で確認できます。
...
プレビューはコンテナ起動時に自動で開始され、`http://localhost:8000` で確認できます。
